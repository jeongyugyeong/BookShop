...
public class BoardDAO {
	...
	public ArrayList<BoardEntity> getBoardList() {	
		...
		String SQL = "select * from board order by ref desc, step";
		try {
	...
	public boolean insertDB(BoardEntity board) {
		...
		try {
			...
			sql ="insert into board values(?, ?, ?, ?, sysdate(), ?, 0, ?, 0, 0, 0)";	
			pstmt = con.prepareStatement(sql);
	...
	public boolean insertReply(BoardEntity board) {
		boolean success = false; 
		String sql;
		int num =0, step=0; 
		connect();		
		...
		try {
			sql = "select min(step) as tstep from board "
			...
			if(step > 0){
				...
				sql = "select max(step) as tstep from board where ref = ? ";
				pstmt = con.prepareStatement(sql);
				pstmt.setInt(1,  board.getRef());
				rs = pstmt.executeQuery(); 
				if(rs.next()){
			...
			//num (글번호) 값을 구한다. 
			sql = "select max(num) as tnum from board";
			pstmt = con.prepareStatement(sql);
			rs = pstmt.executeQuery();
			if(rs.next()){
			...
			for(int r=depth-1; r>=0; r--){
				sql = "select max(step) as tstep from board "
				...
				sql = "update board set childCount = childCount + 1 "
					+ " where ref = ? and depth = ? and step = ? ";
				pstmt = con.prepareStatement(sql);
				pstmt.setInt(1,  board.getRef());
				pstmt.setInt(2, r);
	...
	// 페이징 기법 
	public ArrayList<BoardEntity> getBoardList(int startRecord, int pageRecordNo) {	
		connect();
		ArrayList<BoardEntity> list = new ArrayList<BoardEntity>();
		
		String sql = "select * from board order by ref desc, step limit " 
